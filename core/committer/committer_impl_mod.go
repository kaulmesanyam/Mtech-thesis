...
func (lc *LedgerCommitter) processBlockWithDAG(blockAndPvtData *ledger.BlockAndPvtData,
	...
	if len(metadata.Metadata) <= common.BlockMetadataIndex_TRANSACTIONS_FILTER {
		metadata.Metadata = append(metadata.Metadata, []byte{})
	}

	txFilter := protoutil.NewTxValidationFlagsSetValue(len(blockAndPvtData.Block.Data.Data), peer.TxValidationCode_VALID)
	...
	for txID, node := range dag.Nodes {
		...
		if !dag.IsValid(txID) {
			...
			txFilter.SetFlag(txIndex, peer.TxValidationCode_MVCC_READ_CONFLICT)
		}
	}
