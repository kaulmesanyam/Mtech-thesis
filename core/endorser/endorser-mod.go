...
// Metrics keeps track of all metrics for the Endorser service
type Metrics struct {
...
func (e *Endorser) extractTransactionDependencies(simResult *ledger.TxSimulationResults) (map[string][]byte, error) {
	...
	if simResult.PubSimulationResults != nil {
		for _, nsRWSet := range simResult.PubSimulationResults.NsRwset {
			...
			for _, write := range nsRWSet.Rwset.Writes {
	...
	if simResult.PvtSimulationResults != nil {
		for _, pvtRWSet := range simResult.PvtSimulationResults.NsPvtRwset {
			...
			for _, collection := range pvtRWSet.CollectionPvtRwset {
				...
				for _, write := range collection.Rwset.Writes {
				...
				for _, read := range collection.Rwset.Reads {
...
func (e *Endorser) ProcessProposalSuccessfullyOrError(up *UnpackedProposal) (*pb.ProposalResponse, error) {
	...
	// Create a custom metadata to include dependency information
	dependencyInfo := &pb.Response{
		Status: 200,
		Message: fmt.Sprintf("DependencyInfo:HasDependency=%v,DependentTxID=%s,ExpiryTime=%d",
			hasDependency, dependentTxID, time.Now().Add(e.EndorsementExpiryDuration).Unix()),
	}
